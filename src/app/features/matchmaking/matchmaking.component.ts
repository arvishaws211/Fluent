    ...
    <div class="matchmaking-container animate-fade-in">
      <header class="page-header">
        ...
        <p class="text-secondary">
          Vertex AI synthesizes attendee profiles and sponsor missions to surface high-value
          introductions — all without exposing your identity to the model.
        </p>
      </header>
      ...
      <div class="matchmaking-layout">
        <main class="results-section" aria-labelledby="results-heading">
          ...
          @if (isProcessing()) {
            <div class="status-bar glass" aria-live="polite">
          ...
          @if (error()) {
            <div class="error-bar glass" role="alert"><strong>Heads up.</strong> {{ error() }}</div>
          }

          @if (!isProcessing() && matches().length) {
            <div class="results-grid">
              @for (match of matches(); track match.partnerName) {
                <article
                  class="match-card glass-card"
                  [attr.aria-label]="'Match: ' + match.partnerName"
                >
                  <header class="match-header">
                    <span class="match-badge" [class]="match.type">{{
                      match.type | titlecase
                    }}</span>
                    <span
                      class="match-score"
                      [attr.aria-label]="
                        'Relevance ' + (match.relevanceScore * 100).toFixed(0) + ' percent'
                      "
                    >
          ...
          @if (!isProcessing() && !matches().length && !error()) {
            <p class="empty-state">
              Add a few interests on the right, then tap <em>Update AI agent</em> to generate fresh
              matches.
            </p>
          }
        </main>
...
export class MatchmakingComponent {
  ...
  toggleInterest(tag: string): void {
    ...
    this.selected.set(current.includes(tag) ? current.filter((t) => t !== tag) : [...current, tag]);
  }
