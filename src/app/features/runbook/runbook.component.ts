...
@Component({
  ...
  template: `
    <div class="runbook-container animate-fade-in">
      <header class="page-header">
        <div class="header-content">
          ...
          <p class="text-secondary">
            Operational source of truth synced via Firestore. Edits propagate to every staff device in real time,
            governed by Firestore security rules and App Check.
          </p>
        </div>
      ...
      <form *ngIf="showNew()" class="new-task-form glass-card" (ngSubmit)="createTask()" aria-labelledby="new-task-heading">
        <h3 id="new-task-heading">Add task</h3>
        <div class="grid">
          <div class="form-group">
            <label for="t-title">Title</label>
            <input id="t-title" class="glass-input" [(ngModel)]="draft.title" name="title" required />
          </div>
          <div class="form-group">
            <label for="t-owner">Assigned to</label>
            <input id="t-owner" class="glass-input" [(ngModel)]="draft.assignedTo" name="assignedTo" required />
          </div>
          <div class="form-group">
            <label for="t-priority">Priority</label>
            <select id="t-priority" class="glass-input" [(ngModel)]="draft.priority" name="priority">
        ...
        <p class="error" *ngIf="lastError()" role="alert">{{ lastError() }}</p>
      </form>

      <main class="tasks-table glass-card" aria-labelledby="tasks-heading">
        <h3 id="tasks-heading" class="visually-hidden">Tasks</h3>
        <table>
          <caption class="visually-hidden">Live staff runbook tasks</caption>
          <thead>
            <tr>
          ...
          <tbody>
            <tr *ngFor="let task of tasks()" class="task-row">
              ...
              <td>
                <label class="visually-hidden" [attr.for]="'status-' + task.id">Status for {{ task.title }}</label>
      ...
      <footer class="access-warning glass" role="note">
        ...
        Visible only to authenticated users with the <code>staff</code> or <code>sponsor</code> custom claim.
        Enforced by Firestore security rules.
      </footer>
    </div>
  `,
  styles: [`
    .runbook-container { max-width: 1300px; margin: 0 auto; }
    .visually-hidden {
    ...
    .pulse-dot { width: 8px; height: 8px; background: var(--accent); border-radius: 50%; animation: pulse-sync 2s infinite; }
    ...
    .runbook-stats {
      ...
      background: var(--glass-border); padding: 0; overflow: hidden; margin-bottom: 24px;
    }
    .stat { background: var(--bg-card); padding: 24px; display: flex; flex-direction: column; align-items: center; }
    .stat .label { font-size: .8rem; color: var(--text-muted); margin-bottom: 8px; }
    .stat .value { font-size: 1.5rem; font-weight: 700; }
    .stat .value.danger { color: var(--danger); }

    .new-task-form { padding: 24px; margin-bottom: 24px; }
    .new-task-form .grid { display: grid; grid-template-columns: 2fr 2fr 1fr 1fr; gap: 16px; margin-bottom: 16px; }
    .form-group label { display: block; font-size: .85rem; color: var(--text-secondary); margin-bottom: 4px; }
    .glass-input {
    ...
    .tasks-table { padding: 0; overflow-x: auto; }
    table { width: 100%; border-collapse: collapse; text-align: left; }
    th { padding: 16px 24px; font-size: .85rem; color: var(--text-muted); border-bottom: 1px solid var(--glass-border); font-weight: 500; }
    td { padding: 16px 24px; border-bottom: 1px solid var(--glass-border); font-size: .95rem; }
    .task-row:hover { background: rgba(255,255,255,.02); }
    .task-title { font-weight: 500; }
    .empty { color: var(--text-muted); text-align: center; padding: 32px; }

    .priority-badge { font-size: .7rem; font-weight: 700; text-transform: uppercase; padding: 2px 8px; border-radius: var(--radius-sm); }
    .priority-badge.high { background: rgba(239,68,68,.1); color: var(--danger); }
    .priority-badge.medium { background: rgba(245,158,11,.1); color: var(--warning); }
    .priority-badge.low { background: rgba(16,185,129,.1); color: var(--accent); }

    .timestamp { font-size: .8rem; color: var(--text-muted); }
    ...
    @media (max-width: 768px) {
      .runbook-stats { grid-template-columns: repeat(2, 1fr); }
      .new-task-form .grid { grid-template-columns: 1fr; }
    }
  `],
})
